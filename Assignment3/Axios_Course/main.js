...
// PUT/PATCH REQUEST
function updateTodo() {
    // PUT - Used to replace the item (usually)
    // axios
    ...
    // PUT - Used to update the item (single property usually)
...
function errorHandling() {
    axios
        .get('https://jsonplaceholder.typicode.com/todoss', {
            ...
            //   return status < 500; // Reject only if status code in 500+ series
            // }
        })
        .then(res => showOutput(res))
        .catch(err => {
            if (err.response) {
                // Response not ok i.e. not 200 series
